...
function buildThumbnails() {
    ...
    pages.forEach((page, index) => {
        ...
        `;
        item.addEventListener('click', () => loadPage(index));
...
function drawResizeHandles(x, y, w, h) {
    ...
    // Middle handles (left, right, top, bottom)
    ctx.fillRect(x - hs/2, y + h/2 - hs/2, hs, hs);     // ML (middle-left)
    ctx.fillRect(x + w - hs/2, y + h/2 - hs/2, hs, hs); // MR (middle-right)
    ctx.fillRect(x + w/2 - hs/2, y - hs/2, hs, hs);     // MT (middle-top)
    ctx.fillRect(x + w/2 - hs/2, y + h - hs/2, hs, hs); // MB (middle-bottom)
}
...
function setupEventListeners() {
    ...
    if (btnFitPage) {
        btnFitPage.addEventListener('click', () => { fitToPage(); renderCanvas(); });
    }
...
function handleMouseMove(e) {
    ...
    // Resizing
    if (isResizing && dragField && resizeHandle) {
        const dx = (x - dragStart.x) / (DPI_SCALE * zoom);
        const dy = (y - dragStart.y) / (DPI_SCALE * zoom);
        
        // Adjust rect based on which handle
        const r = dragField.rect;
        if (resizeHandle === 'tl') {
            r[0] += dx; r[1] += dy;
            ...
            // Middle-left: only adjust left edge (horizontal-only)
            r[0] += dx;
        } else if (resizeHandle === 'mr') {
            // Middle-right: only adjust right edge (horizontal-only)
            r[2] += dx;
        } else if (resizeHandle === 'mt') {
            // Middle-top: only adjust top edge (vertical-only)
            r[1] += dy;
        } else if (resizeHandle === 'mb') {
            // Middle-bottom: only adjust bottom edge (vertical-only)
            r[3] += dy;
        }
        
        // Ensure min size
        if (r[2] - r[0] < 10) r[2] = r[0] + 10;
        if (r[3] - r[1] < 8) r[3] = r[1] + 8;
        
        recordFieldResize(dragField, r);
        dragStart = { x, y };
        markUnsaved();
        requestRender();
        updatePropertiesPanel();
...
function setZoom(newZoom) {
    ...
    zoom = Math.max(0.25, Math.min(2.0, newZoom));
    document.getElementById('zoomLevel').textContent = Math.round(zoom * 100) + '%';
    renderCanvas();
...
function updateFieldList() {
    ...
    if (fieldSortOrder === 'alphabetic') {
        ...
        // Sort by tab order (stored as tabindex) or by position if no tab order
        pageFields.sort((a, b) => (a.tabindex || 9999) - (b.tabindex || 9999));
    }
    // 'default' keeps original position order
    ...
    // Build HTML for field list items
    let html = '';
    if (filteredFields.length === 0) {
        ...
        filteredFields.forEach((field, index) => {
            ...
            const tabNum = showTabNumbers ? `<span class="tab-number">${index + 1}</span>` : '';
            ...
            html += `
                <div class="field-list-item ${isSelected ? 'selected' : ''}" 
                    ...
                    <div class="field-info">
                        ...
                        <div class="field-meta">${typeName}</div>
                    </div>
                </div>
            `;
        });
    ...
    const listCount = document.getElementById('fieldListCount');
    if (listCount) {
        listCount.textContent = pageFields.length;
    }
    ...
    container.querySelectorAll('.field-list-item').forEach(item => {
        ...
        item.addEventListener('dragstart', (e) => {
            ...
            if (field && selectedFields.includes(field) && selectedFields.length > 1) {
                ...
                const names = selectedFields.map(f => f.name).join(',');
                e.dataTransfer.setData('text/plain', 'MULTI:' + names);
                
                // Add dragging class to all selected items
                selectedFields.forEach(f => {
                    const el = container.querySelector(`.field-list-item[data-field-name="${f.name.replace(/"/g, '\\"')}"]`);
        ...
        item.addEventListener('dragend', () => {
            ...
            container.querySelectorAll('.field-list-item').forEach(el => el.classList.remove('dragging'));
        });
        ...
        item.addEventListener('drop', (e) => {
            ...
            const targetName = item.dataset.fieldName;
            
            if (data.startsWith('MULTI:')) {
                const draggedNames = data.substring(6).split(',');
                
                // If dropped on one of the selected items, do nothing
                if (draggedNames.includes(targetName)) return;
                
                // Collect valid move items
                const validMoveItems = [];
                const fieldsMap = new Map();
                fields.forEach((f, i) => fieldsMap.set(f.name, { field: f, index: i }));
                
                for (const name of draggedNames) {
                    if (fieldsMap.has(name)) {
                        validMoveItems.push(fieldsMap.get(name));
                    }
                }
                
                // Sort them by original index
                validMoveItems.sort((a, b) => a.index - b.index);
                const moveObjects = validMoveItems.map(item => item.field);
                
                // Remove dragged fields from array
                const newFields = fields.filter(f => !draggedNames.includes(f.name));
                
                // Find new target index
                let newTargetIdx = newFields.findIndex(f => f.name === targetName);
                
                if (newTargetIdx !== -1) {
                    // Insert at target index
                    newFields.splice(newTargetIdx, 0, ...moveObjects);
                ...
                const draggedName = data;
                if (draggedName === targetName) return;
                
                // Find field indices in the main fields array
                const draggedIdx = fields.findIndex(f => f.name === draggedName);
                const targetIdx = fields.findIndex(f => f.name === targetName);
                
                if (draggedIdx !== -1 && targetIdx !== -1) {
                    // Move the dragged field to the target position
                    const [draggedField] = fields.splice(draggedIdx, 1);
                    fields.splice(targetIdx, 0, draggedField);
...
function loadCalcToModal(field) {
    ...
    if (calc && calc.type) {
        ...
        // Load readOnly: prefer calc's own setting, then fall back to
        // the field's actual readOnly flag from the PDF.  The HTML default
        // is "checked", which is misleading for fields that aren't really
        // read-only — always set it explicitly here.
        {
            const roCheck = document.getElementById('calcReadOnly');
            if (roCheck) {
                if (calc.readOnly !== undefined) {
                    roCheck.checked = calc.readOnly;
                } else {
                    // Extracted calc doesn't carry readOnly — use field property
                    roCheck.checked = !!field.readOnly;
                }
            }
        }
    } else {
        calcType.value = '';
        // No calculation — reset readOnly checkbox to match field property
        const roCheck = document.getElementById('calcReadOnly');
        if (roCheck) roCheck.checked = !!field.readOnly;
    }
