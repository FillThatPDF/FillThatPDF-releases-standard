...
function openPropertiesModal(field) {
    ...
    if (field.type === 'link') {
        ...
        if (typeof hypMgrEditDest === 'function') hypMgrEditDest(field.name);
        return;
    }
        ...
        case 'apply-properties': {
            ...
            if (isMulti && multiFields) {
                ...
                multiFields.forEach(fData => {
                    ...
                    if (!f) return;
                    if (!f.style) f.style = {};
                    Object.assign(f, updates.props);
                    Object.assign(f.style, updates.props);
                    changes.styled[f.name] = { ...(changes.styled[f.name] || {}), ...updates.props };
                });
            } else if (fieldObj) {
                // Single-edit
...
function applyGlobalStyles(payload) {
    const { applyTo, fontFamily, fontSize, fontColor, fillColor, transparent,
            textAlign, checkboxStyle } = payload;
