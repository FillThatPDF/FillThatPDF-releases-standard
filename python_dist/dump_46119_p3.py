...
def dump_p3():
    ...
    with pdfplumber.open(pdf_path) as pdf:
        ...
        print(f"--- Page 3 (index 2) Size: {page.width}x{page.height} ---")
        ...
        for l in all_h_lines:
            ...
            print(f"Y={l['top']:.1f} X={l['x0']:.1f}-{l['x1']:.1f} W={l['x1']-l['x0']:.1f} [{src}]")

        print("\n--- All Words ---")
        words = page.extract_words()
        words.sort(key=lambda w: (w['top'], w['x0']))
        for w in words:
            print(f"Y={w['top']:.1f} X={w['x0']:.1f}-{w['x1']:.1f} '{w['text']}'")
