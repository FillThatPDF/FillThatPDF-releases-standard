...
_PAGE_INDEX_CACHE = {}  # pdf_pike id -> {objgen: page_index}

def _get_page_index_map(pdf_pike):
    """Build/reuse a {objgen: page_index} map for O(1) page lookups.
    Cached per-pdf because InDesign docs can have 1000+ named dests."""
    key = id(pdf_pike)
    m = _PAGE_INDEX_CACHE.get(key)
    if m is None:
        m = {p.objgen: i for i, p in enumerate(pdf_pike.pages)}
        _PAGE_INDEX_CACHE[key] = m
    return m
...
def _resolve_dest_to_page_index(dest, pdf_pike) -> int:
    ...
    Handles both legacy /Dests dict and modern /Names/Dests name tree (incl.
    trees with /Kids, which is what InDesign exports with many named destinations)."""
    if isinstance(dest, (pikepdf.String, pikepdf.Name)):
        try:
            ...
            if '/Dests' in pdf_pike.Root:
                try:
                    resolved_dest = pdf_pike.Root.Dests.get(key)
                except Exception:
                    resolved_dest = None

            # Modern: catalog /Names/Dests name tree (may be leaf or have /Kids)
            if not resolved_dest and '/Names' in pdf_pike.Root and '/Dests' in pdf_pike.Root.Names:
                dests_root = pdf_pike.Root.Names.Dests
                # pikepdf.NameTree transparently walks /Kids branches
                try:
                    nt = pikepdf.NameTree(dests_root)
                    resolved_dest = nt.get(key)
                except Exception:
                    resolved_dest = None
                # Fallback: manual recursive walk in case NameTree is unavailable
                if not resolved_dest:
                    resolved_dest = _walk_name_tree(dests_root, key)
    ...
    if isinstance(dest, pikepdf.Array) and len(dest) > 0:
        ...
        try:
            if isinstance(page_ref, int):
                return page_ref
            page_map = _get_page_index_map(pdf_pike)
...
def _walk_name_tree(node, target_key):
    """Manually walk a PDF name tree looking for target_key.
    Fallback for when pikepdf.NameTree is unavailable or errors."""
    try:
        # Leaf node: has /Names = [key1, val1, key2, val2, ...]
        if '/Names' in node:
            names = node['/Names']
            for i in range(0, len(names), 2):
                if str(names[i]) == target_key:
                    return names[i + 1]
        # Branch node: has /Kids = [child1, child2, ...]
        if '/Kids' in node:
            for kid in node['/Kids']:
                # Optional bounds-check via /Limits to skip unrelated branches
                try:
                    if '/Limits' in kid:
                        lo, hi = str(kid['/Limits'][0]), str(kid['/Limits'][1])
                        if not (lo <= target_key <= hi):
                            continue
                except Exception:
                    pass
                found = _walk_name_tree(kid, target_key)
...
def _parse_calculate_js(js_str: str, field_name: str = None) -> dict:
    ...
    # --- Builder-pattern recognizer ---------------------------------------
    # build_calculate_js() emits canonical shapes like:
    #   var v0 = Number(this.getField("X").value) || 0;
    #   var v1 = Number(this.getField("Y").value) || 0;
    #   event.value = 12.5 * (v0);            # CONSTANT_MULTIPLY
    #   event.value = v0 * v1;                # MULTIPLY
    #   event.value = v0 - v1 - v2;           # SUBTRACT
    #   var divisor = v1 * v2;
    #   event.value = divisor !== 0 ? v0 / divisor : 0;   # DIVIDE
    # Optionally followed by: event.value = Number(event.value).toFixed(D);
    # Recognize these BEFORE the is_complex check so they round-trip cleanly.
    _var_decls = re.findall(
        r'var\s+v(\d+)\s*=\s*Number\s*\(\s*(?:this\.)?getField\s*\(\s*["\']([^"\']+)["\']\s*\)\s*\.value\s*\)\s*\|\|\s*0\s*;',
        js_str
    )
    if _var_decls:
        _idx_to_name = {int(i): n for i, n in _var_decls}
        _ordered_sources = [_idx_to_name[k] for k in sorted(_idx_to_name.keys())]

        # Extract decimals from trailing toFixed, then strip it
        _decimals = None
        _stripped = js_str
        _tofixed = re.search(
            r'event\.value\s*=\s*Number\s*\(\s*event\.value\s*\)\s*\.toFixed\s*\(\s*(\d+)\s*\)\s*;?',
            _stripped
        )
        if _tofixed:
            try:
                _decimals = int(_tofixed.group(1))
            except ValueError:
                _decimals = None
            _stripped = _stripped[:_tofixed.start()] + _stripped[_tofixed.end():]

        # Find the main event.value assignment (ignore var lines)
        _ev_assigns = re.findall(r'event\.value\s*=\s*([^;]+);', _stripped)
        _main_expr = _ev_assigns[-1].strip() if _ev_assigns else ""

        def _finalize(result):
            if _decimals is not None:
                result["decimals"] = _decimals
            return result

        # CONSTANT_MULTIPLY: "<CONST> * (v0)" or "<CONST> * (v0 * v1 * ...)"
        m = re.fullmatch(
            r'\s*([\d.]+)\s*\*\s*\(\s*((?:v\d+\s*\*\s*)*v\d+)\s*\)\s*',
            _main_expr
        )
        if m and _ordered_sources:
            try:
                _const = float(m.group(1))
            except ValueError:
                _const = None
            _vars_used = re.findall(r'v(\d+)', m.group(2))
            _srcs = [_idx_to_name[int(i)] for i in _vars_used if int(i) in _idx_to_name]
            if _const is not None and _srcs:
                return _finalize({
        ...
        # Mirror: "(v0) * <CONST>"
        m = re.fullmatch(
            r'\s*\(\s*((?:v\d+\s*\*\s*)*v\d+)\s*\)\s*\*\s*([\d.]+)\s*',
            _main_expr
        )
        if m and _ordered_sources:
            try:
                _const = float(m.group(2))
            except ValueError:
                _const = None
            _vars_used = re.findall(r'v(\d+)', m.group(1))
            _srcs = [_idx_to_name[int(i)] for i in _vars_used if int(i) in _idx_to_name]
            if _const is not None and _srcs:
                return _finalize({
                    "type": "CONSTANT_MULTIPLY",
        ...
        # MULTIPLY: "v0 * v1 * ..."
        if re.fullmatch(r'\s*v\d+(?:\s*\*\s*v\d+)+\s*', _main_expr):
            _vars_used = re.findall(r'v(\d+)', _main_expr)
            _srcs = [_idx_to_name[int(i)] for i in _vars_used if int(i) in _idx_to_name]
            if _srcs:
                return _finalize({"type": "MULTIPLY", "sources": _srcs})
        ...
        if re.fullmatch(r'\s*v\d+(?:\s*-\s*v\d+)+\s*', _main_expr):
            ...
            _srcs = [_idx_to_name[int(i)] for i in _vars_used if int(i) in _idx_to_name]
            if _srcs:
                return _finalize({"type": "SUBTRACT", "sources": _srcs})

        # DIVIDE: "divisor !== 0 ? v0 / divisor : 0" with "var divisor = v1 * v2..."
        _div_match = re.search(
            r'var\s+divisor\s*=\s*((?:v\d+\s*\*\s*)*v\d+)\s*;', _stripped)
        if _div_match and re.search(
            r'divisor\s*!==\s*0\s*\?\s*v(\d+)\s*/\s*divisor\s*:\s*0', _main_expr
        ):
            _num_match = re.search(
                r'divisor\s*!==\s*0\s*\?\s*v(\d+)\s*/\s*divisor\s*:\s*0', _main_expr)
            _num_idx = int(_num_match.group(1))
            _denom_vars = re.findall(r'v(\d+)', _div_match.group(1))
            _all = [_num_idx] + [int(i) for i in _denom_vars]
            _srcs = [_idx_to_name[k] for k in _all if k in _idx_to_name]
            if _srcs:
                return _finalize({"type": "DIVIDE", "sources": _srcs})
