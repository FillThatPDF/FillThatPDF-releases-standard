...
class BoxEntryDetector(BaseDetector):
    ...
    def _detect_box_entries(self, page_model: PageModel) -> List[FieldCandidate]:
        ...
        for y_key, vlines in v_groups.items():
            ...
            for group in box_groups:
                ...
                # Count actual boxes by matching spacings (handles
                # paired-wall patterns where len(group)-1 overcounts)
                num_boxes = sum(
                    1 for j in range(len(group) - 1)
                    if abs(group[j + 1]['x'] - group[j]['x'] - box_spacing) <= tolerance
                )
                if num_boxes < 1:
                    num_boxes = len(group) - 1  # fallback
    ...
    def _extract_uniform_groups(
        ...
        Distinguishes two kinds of non-matching gaps:
        * **Wall gaps** (< box_spacing * 0.4): thin shared walls between
          adjacent box cells — bridged transparently within the same group.
        * **Separator gaps** (≥ box_spacing * 0.4 but ≤ box_spacing * 2.5):
          visual separators (dashes, slashes) between digit groups — each
          side becomes its own comb field.
        """
        tolerance = max(3, box_spacing * 0.2)
        wall_gap_threshold = box_spacing * 0.4
        ...
        while i < len(spacings):
            if abs(spacings[i] - box_spacing) <= tolerance:
                ...
                current_sub = [vlines[i]]

                while i < len(spacings) and abs(spacings[i] - box_spacing) <= tolerance:
                    current_sub.append(vlines[i + 1])
                    i += 1

                sub_groups.append(current_sub)

                # Bridge gaps that lead to more matching spacings
                while i < len(spacings) - 1:
                    ...
                    if gap <= box_spacing * 2.5 and i + 1 < len(spacings):
                        ...
                        if abs(next_spacing - box_spacing) <= tolerance:
                            if gap < wall_gap_threshold:
                                # Thin wall gap — continue same sub-group
                                current_sub.append(vlines[i + 1])
                                i += 1
                                while i < len(spacings) and abs(spacings[i] - box_spacing) <= tolerance:
                                    current_sub.append(vlines[i + 1])
                                    i += 1
                                # current_sub is still the same list ref
                                # in sub_groups, so it's updated in place
                            else:
                                # Separator gap — start a new sub-group
                                current_sub = [vlines[i + 1]]
                                i += 1
                                while i < len(spacings) and abs(spacings[i] - box_spacing) <= tolerance:
                                    current_sub.append(vlines[i + 1])
                                    i += 1
                ...
                # Count total boxes across all sub-groups using spacing match
                total_boxes = 0
                for sg in sub_groups:
                    for j in range(len(sg) - 1):
                        if abs(sg[j + 1]['x'] - sg[j]['x'] - box_spacing) <= tolerance:
                            total_boxes += 1

                if total_boxes >= 8:
                    if len(sub_groups) > 1:
                        # Separated comb — emit each sub-group individually
                        for sg in sub_groups:
                            if len(sg) >= 2:
                                box_groups.append(sg)
                    else:
                        box_groups.append(sub_groups[0])
            else:
                i += 1
