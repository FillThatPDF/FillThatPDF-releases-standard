...
class GridGapFill:
    ...
    def _build_grid(
        ...
        for i in range(len(row_ys) - 1):
            ...
            # On pages with real V-lines, rows without local V-lines
            # may be either:
            #   (a) structural gaps (between grid sections) — skip, or
            #   (b) h-line-only table sections on a mixed page — use
            #       h-line junction fallback to find column boundaries.
            # Try the junction fallback first; only skip if no column
            # boundaries can be inferred.
            if not local_v_xs and page_has_v_lines:
                top_junc = self._get_h_line_junctions(
                    y0, h_lines, row_x0, row_x1)
                bot_junc = self._get_h_line_junctions(
                    y1, h_lines, row_x0, row_x1)
                ...
                if shared:
                    ...
                    # No column boundaries found — structural gap, skip.
                    continue
