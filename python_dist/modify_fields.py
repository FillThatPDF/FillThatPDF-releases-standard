...
def _ensure_font_resources(pdf):
    ...
    try:
        ...
        # Core standard fonts (always register these)
        add_font('/Helv', '/Helvetica')
        add_font('/Cour', '/Courier')
        add_font('/TiRo', '/Times-Roman')

        # Bold/italic variants of the 14 standard PDF fonts
        add_font('/Helvetica-Bold',        '/Helvetica-Bold')
        add_font('/Helvetica-Oblique',     '/Helvetica-Oblique')
        add_font('/Helvetica-BoldOblique', '/Helvetica-BoldOblique')
        add_font('/Times-Bold',            '/Times-Bold')
        add_font('/Times-Italic',          '/Times-Italic')
        add_font('/Times-BoldItalic',      '/Times-BoldItalic')
        add_font('/Courier-Bold',          '/Courier-Bold')
        add_font('/Courier-Oblique',       '/Courier-Oblique')
        add_font('/Courier-BoldOblique',   '/Courier-BoldOblique')
        # Note: Acrobat-bundled fonts (MyriadPro, MinionPro) are registered on-demand
        # in apply_styles() only when a field actually uses them, to avoid polluting
        # PDFs that don't use those fonts with stub entries that trigger /BBox errors.
...
def _topological_sort_calculations(calculations: Dict[str, Dict], all_field_names: set, strict: bool = False, return_cycles: bool = False):
    """Topological sort of calculated fields to determine /CO order.

    When ``strict`` is True, raises ValueError on any circular dependency.
    When ``strict`` is False (default), performs a *best-effort* sort:
    fields that can be ordered acyclically come first, and any remaining
    fields (those involved in cycles) are appended at the end in stable
    input order.  A warning describing the cycle is written to stderr so
    the /CO array is still populated — previously a single cycle caused
    the entire /CO build to abort, which made Acrobat skip calculation
    ordering and render calculated fields as invisible.

    When ``return_cycles`` is True, returns a tuple ``(ordered, cyclic_set)``
    where ``cyclic_set`` contains the field names involved in cycles.
    Callers can use the set to neutralise /AA/C on those fields (their
    calculation can never terminate cleanly in Acrobat, which triggers
    /AP regeneration that erases the visibility tint — causing the
    fields to appear blank).
    """
    ...
    if len(result) != len(graph):
        ...
        remaining_set = set(graph.keys()) - set(result)
        if strict:
            raise ValueError(f"Circular dependency detected among fields: {', '.join(remaining_set)}")
        # Best-effort: append cyclic fields in their original input order so
        # the /CO array still covers every calculated field.  Acrobat will
        # still evaluate them; ordering inside the cycle is inherently
        # ambiguous but at least the fields are present.
        try:
            sys.stderr.write(
                f"[calc] Warning: circular dependency detected among "
                f"{len(remaining_set)} calculated fields; appending in "
                f"input order: {', '.join(sorted(remaining_set))}\n"
            )
        except Exception:
            pass
        for target in calculations.keys():
            if target in remaining_set:
                result.append(target)

        if return_cycles:
            return result, remaining_set
...
def _set_link_destination(pdf, annot, dest_type, value):
    """Set the /A (action) or /Dest entry on a /Link annotation.

    dest_type: 'url' | 'goto' | 'goto_remote' | 'dest'
    value:
      - url:         the full URL string (e.g. "https://example.com")
      - goto:        a 0-indexed page number, either as int or numeric string
      - goto_remote: a filesystem path to another PDF
      - dest:        a named destination string
    """
    # Clear any existing action / destination entries so we don't leave
    # stale keys behind when changing destType.
    if '/A' in annot: del annot['/A']
    if '/Dest' in annot: del annot['/Dest']

    if dest_type == 'url':
        annot['/A'] = pikepdf.Dictionary({
            '/Type': pikepdf.Name('/Action'),
            '/S':    pikepdf.Name('/URI'),
            '/URI':  pikepdf.String(str(value))
        })
    elif dest_type == 'goto':
        # The editor stores the page number 0-indexed (see saveLinkDestinationModal)
        try:
            page_idx = int(str(value).strip())
        except (ValueError, TypeError):
            page_idx = 0
        if 0 <= page_idx < len(pdf.pages):
            target_page = pdf.pages[page_idx]
            # /D = [page_ref /Fit] — jumps to top of the page, fit-to-window.
            # pikepdf.Page has .obj for the underlying Dictionary reference.
            page_ref = target_page.obj if hasattr(target_page, 'obj') else target_page
            annot['/A'] = pikepdf.Dictionary({
                '/Type': pikepdf.Name('/Action'),
                '/S':    pikepdf.Name('/GoTo'),
                '/D':    pikepdf.Array([page_ref, pikepdf.Name('/Fit')])
            })
    elif dest_type == 'goto_remote':
        annot['/A'] = pikepdf.Dictionary({
            '/Type': pikepdf.Name('/Action'),
            '/S':    pikepdf.Name('/GoToR'),
            '/F':    pikepdf.String(str(value)),
            '/D':    pikepdf.Array([0, pikepdf.Name('/Fit')])
        })
    elif dest_type == 'dest':
        # Named destination — just a string.  Acrobat resolves it via /Names.
        annot['/Dest'] = pikepdf.String(str(value))

def _create_link_annotation(pdf, page, pdf_rect, dest_type, value, appearance=None):
    """Build a new /Link annotation on `page` with PDF-space rect [x1,y1,x2,y2]
    and the given destination.  Returns the indirect annot."""
    annot_dict = pikepdf.Dictionary({
        '/Type':    pikepdf.Name('/Annot'),
        '/Subtype': pikepdf.Name('/Link'),
        '/Rect':    pikepdf.Array([float(c) for c in pdf_rect]),
        # Border [H V W] — zero-width so the clickable region is invisible
        # (matches what InDesign exports for hyperlinks on images/text).
        '/Border':  pikepdf.Array([0, 0, 0]),
        '/F':       4,  # PDF annotation flag: Print
    })
    annot = pdf.make_indirect(annot_dict)
    _set_link_destination(pdf, annot, dest_type, value)
    _apply_link_appearance(annot, appearance)
...
def _apply_link_appearance(annot, appearance):
    """Write /H, /BS, /C and /Border to a /Link annotation from an appearance dict.
    appearance keys (all optional):
        linkType        'invisible' | 'visible'
        linkHighlight   'N' | 'I' | 'O' | 'P'
        linkBorderWidth int (0 = no border)
        linkBorderStyle 'S' | 'D'
        linkBorderColor '#rrggbb'
    """
    if not appearance:
        return

    # Highlight mode (/H)
    h_mode = appearance.get('linkHighlight', 'I')
    annot['/H'] = pikepdf.Name('/' + h_mode)

    link_type   = appearance.get('linkType', 'invisible')
    border_w    = int(appearance.get('linkBorderWidth', 0))
    border_style = appearance.get('linkBorderStyle', 'S')
    color_hex   = appearance.get('linkBorderColor', '')

    if link_type == 'visible' and border_w > 0 and color_hex:
        ...
        if len(color_hex) == 6:
            ...
            annot['/C'] = pikepdf.Array([r, g, b])
        else:
            annot['/C'] = pikepdf.Array([0, 0, 1])  # fallback: blue

        # /BS — border style dictionary
        bs = pikepdf.Dictionary()
        bs['/Type'] = pikepdf.Name('/Border')
        bs['/W']    = float(border_w)
        bs['/S']    = pikepdf.Name('/' + border_style)
        annot['/BS'] = bs

        # Legacy /Border [H V W] — keep consistent
        annot['/Border'] = pikepdf.Array([0, 0, float(border_w)])
    else:
        # Invisible: clear color and border
        if '/C'      in annot: del annot['/C']
...
def apply_field_changes(pdf_path: str, output_path: str, changes: Dict[str, Any], settings: Dict[str, Any] = {}) -> dict:
    ...
    try:
        ...
        for fd in changes.get("created", []):
            try:
                ...
                if f_type == "checkbox": 
                    ...
                    # Style defaults from settings — prefer checkbox-specific bg, fall back to generic
                    bg_color = settings.get('checkbox_bg_color',
                               settings.get('checkboxBgColor',
                               settings.get('field_background_color', '#FFFFFF')))
                    bg_rgb = None  # Default: transparent
                    if bg_color and str(bg_color).lower() not in ['none', 'transparent', 'null', '']:
                        try:
                            bg_rgb = [int(bg_color.lstrip('#')[i:i+2], 16) / 255.0 for i in (0, 2, 4)]
                        except (ValueError, IndexError):
                            bg_rgb = None
                    
                    border_color = settings.get('checkbox_border_color',
                                   settings.get('checkboxBorderColor',
                                   settings.get('field_border_color', 'none')))
                    border_w = float(settings.get('field_border_thickness', 0))
                    border_rgb = None
                    if border_color and border_color.startswith('#') and len(border_color.lstrip('#')) == 6:
                        border_rgb = [int(border_color.lstrip('#')[i:i+2], 16) / 255.0 for i in (0, 2, 4)]
                    ...
                    # MK for native renderers — only set /BG if not transparent
                    if '/MK' not in widget: widget['/MK'] = pikepdf.Dictionary()
                    if bg_rgb:
                        widget['/MK']['/BG'] = pikepdf.Array(bg_rgb)
                    else:
                        # Transparent — ensure no /BG key so viewer doesn't draw a background
                        if '/BG' in widget.get('/MK', {}): del widget['/MK']['/BG']
        ...
        def apply_styles(annot, s):
            ...
            if f_size is not None or f_color or f_family:
                ...
                if f_family: font_tag = {
                    # Standard PDF fonts (14 core fonts — supported everywhere)
                    'Helvetica':            'Helv',
                    ...
                    # Acrobat-bundled fonts (work in Acrobat & viewers with Adobe fonts)
                    'MyriadPro-Regular':    'MyriadPro-Regular',
                    'MyriadPro-Bold':       'MyriadPro-Bold',
                    'MyriadPro-It':         'MyriadPro-It',
                    'MyriadPro-BoldIt':     'MyriadPro-BoldIt',
                    'MinionPro-Regular':    'MinionPro-Regular',
                    'MinionPro-Bold':       'MinionPro-Bold',
                    'MinionPro-It':         'MinionPro-It',
                    'MinionPro-BoldIt':     'MinionPro-BoldIt',
                    # Legacy aliases
                    'Arial':                'Helv',
                ...
                # If this is an Acrobat-bundled font, register it on-demand in the
                # AcroForm DR so Acrobat can resolve it — but only for this PDF,
                # and only when the font is actually being used (avoids /BBox errors
                # on PDFs that don't use these fonts).
                ACROBAT_FONTS = {
                    'MyriadPro-Regular', 'MyriadPro-Bold', 'MyriadPro-It', 'MyriadPro-BoldIt',
                    'MinionPro-Regular', 'MinionPro-Bold', 'MinionPro-It', 'MinionPro-BoldIt',
                }
                if font_tag in ACROBAT_FONTS:
                    try:
                        # Use a distinct name (_af) to avoid shadowing the outer-scope
                        # `acroform` variable — apply_styles is a nested function and
                        # Python would make `acroform` local to the whole function if
                        # we assigned to it here, causing UnboundLocalError at the
                        # `acroform['/NeedAppearances']` lines above.
                        _af = pdf.Root.get('/AcroForm')
                        if _af is not None:
                            _dr = _af.get('/DR')
                            if _dr is None:
                                _af['/DR'] = pikepdf.Dictionary()
                                _dr = _af['/DR']
                            _fdict = _dr.get('/Font')
                            if _fdict is None:
                                _dr['/Font'] = pikepdf.Dictionary()
                                _fdict = _dr['/Font']
                            _key = pikepdf.Name(f'/{font_tag}')
                            if _key not in _fdict:
                                _fdict[_key] = pdf.make_indirect(pikepdf.Dictionary({
                                    '/Type':     pikepdf.Name('/Font'),
                                    '/Subtype':  pikepdf.Name('/Type1'),
                                    '/BaseFont': pikepdf.Name(f'/{font_tag}'),
                                    '/Encoding': pikepdf.Name('/WinAnsiEncoding'),
                                }))
                    except Exception:
                        pass  # Non-fatal — Acrobat will still attempt to resolve the font
            ...
            # Tooltip
            # /TU can live on the widget annotation OR on a parent field node
            # (common in hierarchical AcroForms). We must read from and write to
            # the actual node that holds /TU, not just the annotation object.
            if "tooltip" in s:
                if s["tooltip"]:
                    # Write to the annotation; also write to any parent that
                    # already owns /TU so readers see the new value.
                    annot['/TU'] = pikepdf.String(s["tooltip"])
                    node = annot
                    while '/Parent' in node:
                        node = node['/Parent']
                        if '/TU' in node:
                            node['/TU'] = pikepdf.String(s["tooltip"])
                else:
                    # Clear from the annotation itself …
                    if '/TU' in annot:
                        del annot['/TU']
                    # … and walk up every parent node to clear it there too
                    node = annot
                    while '/Parent' in node:
                        node = node['/Parent']
                        if '/TU' in node:
        ...
        # --- Robust tooltip-clear sweep ---
        # Some PDFs store /TU on a parent field node whose children share a
        # partial name. The normal apply_styles walk from a widget may miss
        # it if /Parent references are odd, so do an explicit tree sweep.
        clear_tooltip_names = {
            name_updates.get(n, n)
            for n, s in styled_by_name.items()
            if "tooltip" in s and not s["tooltip"]
        }
        set_tooltip_map = {
            name_updates.get(n, n): s["tooltip"]
            for n, s in styled_by_name.items()
            if s.get("tooltip")
        }
        if clear_tooltip_names or set_tooltip_map:
            try:
                acroform_fields = acroform.get('/Fields', [])
                def _walk_fields(fields, inherited_name=""):
                    for f in fields:
                        try:
                            fobj = pdf.get_object(f.objgen) if f.is_indirect else f
                        except Exception:
                            fobj = f
                        partial = str(fobj['/T']) if '/T' in fobj else ""
                        # Candidates: this node's partial name, the fully-qualified
                        # dotted name, and the inherited name (for widgets w/o /T).
                        full = f"{inherited_name}.{partial}" if inherited_name and partial else (partial or inherited_name)
                        candidates = {c for c in (partial, full, inherited_name) if c}
                        matched_clear = candidates & clear_tooltip_names
                        matched_set = candidates & set(set_tooltip_map.keys())
                        if matched_clear and '/TU' in fobj:
                            del fobj['/TU']
                            sys.stderr.write(f"[tooltip-sweep] cleared /TU on '{next(iter(matched_clear))}'\n")
                        if matched_set:
                            name = next(iter(matched_set))
                            fobj['/TU'] = pikepdf.String(set_tooltip_map[name])
                        if '/Kids' in fobj:
                            _walk_fields(fobj['/Kids'], full or inherited_name)
                _walk_fields(acroform_fields)
            except Exception as e:
                sys.stderr.write(f"[tooltip-sweep] failed: {e}\n")
        ...
        if calculations:
            ...
            for target_name, calc in calculations.items():
                ...
                for _, annot in annot_map[t_name]:
                    ...
                    # Strip format/keystroke validators from calc targets.
                    # Auto-formatting for phone/date/currency assigns a
                    # /AA/K (keystroke) and /AA/F (format) that reject
                    # numeric output of a calc — Acrobat shows
                    # "The value entered does not match the format…"
                    # on every open.  Calc targets receive their value
                    # programmatically, so these formatters are never
                    # useful and actively break the PDF.
                    try:
                        aa = annot.get('/AA')
                        if aa is not None:
                            for k in ('/K', '/F'):
                        ...
                        try:
                            sys.stderr.write(f"[calc] format strip failed for {t_name}: {_fmt_err}\n")
                        except Exception:
                            pass
                    ...
                    if calc.get("readOnly", False):
                        ...
                        # Visibility hint: ReadOnly fields are NOT drawn by
                        # Acrobat's "Highlight Existing Fields" overlay
                        # (that overlay only highlights editable fields).
                        # If the source PDF didn't give this field its own
                        # /MK/BG, the field renders as nothing visible in
                        # Acrobat — the user reports it as "invisible."
                        # Fix: give every readOnly-calc field a subtle
                        # light-blue background so it's visible regardless
                        # of the viewer's highlight-fields setting.  Only
                        # add this when the field has no explicit /MK/BG
                        # so we never overwrite the PDF's original styling.
                        try:
                            existing_mk = annot.get('/MK')
                            has_bg = existing_mk is not None and '/BG' in existing_mk
                            if not has_bg:
                                tint = pikepdf.Array([0.812, 0.890, 0.965])  # light blue
                                if existing_mk is not None:
                                    existing_mk['/BG'] = tint
                                else:
                                    annot['/MK'] = pikepdf.Dictionary({'/BG': tint})
                        except Exception as _bg_err:
                            try:
                                sys.stderr.write(f"[calc] visibility tint failed for {t_name}: {_bg_err}\n")
                            except Exception:
                                pass
        ...
        # Always initialise so Section 10.5 can safely consult it even on
        # saves that have no new calculations.
        cyclic_fields = set()
        if calculations:
            try:
                ...
                # Best-effort sort: never raises; cycles are appended at
                # the end with a stderr warning.  Previously a cycle caused
                # ValueError → /CO never built → NeedAppearances stayed True
                # → Section 10 never ran → calc fields rendered invisible.
                ordered, cyclic_fields = _topological_sort_calculations(
                    merged_calculations, all_names, strict=False, return_cycles=True
                )

                # Neutralise circular-dependency calc fields.
                #
                # Background: a field whose calculation depends (directly or
                # transitively) on itself can never terminate cleanly.  When
                # Acrobat runs its OpenAction ``calculateNow()`` pass it
                # throws on these fields, triggers an appearance-regeneration
                # for the affected widgets, and the regenerated /AP overwrites
                # the visibility tint we carefully applied below in Section 10.
                # The user sees those specific fields as blank rectangles even
                # though every surrounding field renders correctly.
                #
                # Fix: for any field in a cycle:
                #   • remove /AA/C (and /AA on parent if only /C remains) so
                #     Acrobat never tries to evaluate a doomed formula;
                #   • drop the ReadOnly flag — the user can now type a value
                #     manually, and Acrobat's highlight-fields overlay will
                #     draw the field like any other editable widget;
                #   • strip any stale /V (leftover from a prior save).
                # The /MK/BG tint set in Section 8.5 stays, so the field
                # remains visually obvious on white paper.
                for cyc_name in cyclic_fields:
                    t_name = name_updates.get(cyc_name, cyc_name)
                    if t_name not in annot_map:
                        continue
                    for _, annot in annot_map[t_name]:
                        try:
                            aa = annot.get('/AA')
                            if aa is not None and '/C' in aa:
                                del aa['/C']
                                if len(aa.keys()) == 0:
                                    del annot['/AA']
                        except Exception:
                            pass
                        # Parent may hold its own /AA/C
                        try:
                            parent = annot.get('/Parent')
                            if parent is not None and not isinstance(parent, pikepdf.Dictionary):
                                try:
                            ...
                            if parent is not None and isinstance(parent, pikepdf.Dictionary):
                                p_aa = parent.get('/AA')
                                if p_aa is not None and '/C' in p_aa:
                                    del p_aa['/C']
                                    if len(p_aa.keys()) == 0:
                                        del parent['/AA']
                        except Exception:
                            pass
                        # Clear ReadOnly so Acrobat treats it as a normal
                        # editable field (the highlight overlay will now
                        # cover it, /AP will regenerate normally).
                        try:
                        ...
                        try:
                            ...
                            pass
                        # Drop any /AP Section 8.5/10 may have already
                        # baked with the *pale* tint (0.91/0.94/0.996) —
                        # that stream is indistinguishable from white
                        # paper.  Re-building via Section 10.5 later
                        # uses the darker 0.82/0.89/0.98 tint.
                        try:
                            if '/AP' in annot:
                                del annot['/AP']
                        except Exception:
                            pass
                        # Force the /MK/BG to the darker visibility tint
                        # (Section 8.5 may have set the pale value before
                        # we decided this field is cyclic).  Section 10
                        # reads /MK/BG directly when building /AP.
                        try:
                            mk = annot.get('/MK')
                            darker = pikepdf.Array([0.812, 0.890, 0.965])
                            if mk is not None:
                                mk['/BG'] = darker
                            else:
                                annot['/MK'] = pikepdf.Dictionary({'/BG': darker})
                        except Exception:
                            pass
                    # Remove from merged_calculations so /CO doesn't
                    # reference a field that no longer has /AA/C.
                    merged_calculations.pop(cyc_name, None)

                if cyclic_fields:
                    try:
                        sys.stderr.write(
                            f"[calc] neutralised {len(cyclic_fields)} circular "
                            f"calc field(s) — /AA/C removed, ReadOnly cleared, "
                            f"tint retained so they remain visible and manually "
                            f"fillable in Acrobat\n"
                        )
                    except Exception:
                        pass

                    # Rebuild ordered list without the neutralised fields
                    ordered = [n for n in ordered if n not in cyclic_fields]
                ...
                # NOTE: do NOT set NeedAppearances=True here — Section 10
                # below will set it to False when /CO is present (so the
                # background-fill AP streams we build there don't get
                # overwritten by Acrobat's appearance-regeneration pass,
                # which was the root cause of invisible calculated fields).
            except Exception as e:
                result["calc_error"] = str(e)
                try:
                    sys.stderr.write(f"[calc] /CO build failed: {e}\n")
                except Exception:
                    pass
        ...
        # 8c. Hyperlink destination edits, deletions, and new links.
        # These come from the v1.1.3 hyperlinks manager / right-click menu.
        #   changes.hyperlinks:         { name: {destType, url, objgen, rect, page} }
        #   changes.hyperlinks_deleted: [ {name, objgen} ]   # existing links to remove
        #   changes.hyperlinks_created: [ name, ... ]        # names in .hyperlinks that are NEW
        hyperlinks       = changes.get("hyperlinks", {}) or {}
        hyperlinks_del   = changes.get("hyperlinks_deleted", []) or []
        hyperlinks_new   = set(changes.get("hyperlinks_created", []) or [])
        if hyperlinks or hyperlinks_del or hyperlinks_new:
            # Build an objgen -> (page, annot) map over all /Link annotations.
            # objgen is the most reliable identifier across saves.
            link_by_objgen = {}
            for page in pdf.pages:
                if '/Annots' not in page: continue
                for annot in page.Annots:
                    if annot.get('/Subtype') != pikepdf.Name('/Link'): continue
                    if annot.is_indirect:
                        link_by_objgen[tuple(annot.objgen)] = (page, annot)

            # --- Deletions of existing links ---
            for entry in hyperlinks_del:
                og = entry.get("objgen") if isinstance(entry, dict) else None
                if not og: continue
                key = tuple(og)
                if key not in link_by_objgen: continue
                page, annot = link_by_objgen[key]
                if '/Annots' in page:
                    _remove_from_list(page.Annots, annot)
                result["hyperlinks_deleted"] = result.get("hyperlinks_deleted", 0) + 1

            # --- Edits to existing links + new links ---
            for name, payload in hyperlinks.items():
                dest_type = payload.get("destType", "url")
                url       = payload.get("url", "")
                og        = payload.get("objgen")
                is_new    = (name in hyperlinks_new) or (not og)

                if is_new:
                    # Create a fresh link annotation at the stored rect/page
                    page_num = payload.get("page", 0)
                    v_rect   = payload.get("rect")
                    if not v_rect or page_num is None or page_num >= len(pdf.pages):
                        continue
                    page = pdf.pages[page_num]
                    x_off = p_x_offsets[page_num] if page_num < len(p_x_offsets) else 0
                    pdf_rect = [v_rect[0] + x_off,
                                p_heights[page_num] - v_rect[3],
                                v_rect[2] + x_off,
                                p_heights[page_num] - v_rect[1]]
                    try:
                        _create_link_annotation(pdf, page, pdf_rect, dest_type, url,
                                               appearance=payload)
                        result["hyperlinks_created"] = result.get("hyperlinks_created", 0) + 1
                    except Exception as e:
                        sys.stderr.write(f"Warning: failed to create hyperlink '{name}': {e}\n")
                else:
                    # Edit existing link — resolve by objgen
                    key = tuple(og)
                    if key not in link_by_objgen:
                        sys.stderr.write(f"Warning: hyperlink '{name}' objgen {og} not found — skipping\n")
                        continue
                    page, annot = link_by_objgen[key]
                    try:
                        _set_link_destination(pdf, annot, dest_type, url)
                        _apply_link_appearance(annot, payload)
                        result["hyperlinks_edited"] = result.get("hyperlinks_edited", 0) + 1
                    except Exception as e:
                        sys.stderr.write(f"Warning: failed to edit hyperlink '{name}': {e}\n")
        ...
        # Checkbox-specific background — may differ from text-field background.
        # Respects the checkbox_bg_color override AND the transparent guard so
        # that checkboxes honour "Transparent" just like every other field type.
        _cb_bg_raw = settings.get('checkbox_bg_color',
                     settings.get('checkboxBgColor',
                     settings.get('field_background_color', '#FFFFFF')))
        checkbox_bg_rgb = None  # None → transparent (no fill)
        if _cb_bg_raw and str(_cb_bg_raw).lower() not in ['none', 'transparent', 'null', '']:
            try:
                _h = str(_cb_bg_raw).lstrip('#')
                if len(_h) == 6:
                    checkbox_bg_rgb = [int(_h[i:i+2], 16) / 255.0 for i in (0, 2, 4)]
            except Exception:
                checkbox_bg_rgb = None
            ...
            if '/Annots' in page:
                for annot in page.Annots:
                    if annot.get('/Subtype') == pikepdf.Name('/Widget') or annot.get('/Subtype') == '/Widget':
                        ...
                        if '/Btn' in ft:
                            # Always regenerate the appearance stream so that:
                            #   - checkboxes are visible (Acrobat NeedAppearances is
                            #     unreliable for vector-drawn buttons), AND
                            #   - the background respects the checkbox_bg_color setting,
                            #     including the "Transparent" case (checkbox_bg_rgb=None).
                            if '/AP' not in annot or not has_global_border:
                                ...
                                # Use checkbox_bg_rgb (None = transparent, respects setting)
                                appearances = _create_radio_appearance_stream(pdf, rect, ev, checkbox_bg_rgb, style)
                                ap_n = pikepdf.Dictionary()
                                ap_n['/Off'] = appearances['off']
                                ap_n[pikepdf.Name('/' + ev)] = appearances['on']
                                annot['/AP'] = pikepdf.Dictionary({'/N': ap_n})

                                # Sync /MK/BG so native renderers (Preview, etc.) agree
                                if '/MK' not in annot:
                                    annot['/MK'] = pikepdf.Dictionary()
                                if checkbox_bg_rgb:
                                    annot['/MK']['/BG'] = pikepdf.Array(checkbox_bg_rgb)
                                else:
                                    # Transparent — remove /BG so no background is painted
                                    if '/BG' in annot['/MK']:
                                        del annot['/MK']['/BG']
        ...
        # calculation results.
        #
        # ⚠️ Do NOT "simplify" this to `NeedAppearances = True` + `del /AP`
        # for calc fields — that path produces invisible calculated fields
        # in Acrobat (regression we hit in v1.1.6). See v1.1.5 comment above.
        if '/CO' in acroform and len(acroform['/CO']) > 0:
            ap_fields_total = 0       # fields that already had a valid Stream /AP
            ap_fields_created = 0     # fields where we just built a background-fill /AP
            ap_fields_missing = 0     # fields that end up with no /AP at all
            for page in pdf.pages:
                if '/Annots' not in page:
                    continue
                for annot in page.Annots:
                    ...
                    # /Resources is REQUIRED by the PDF spec for Form XObjects —
                    # Acrobat silently skips rendering streams that lack it.
                    ap_stream = pdf.make_indirect(pikepdf.Stream(
                        pdf, ap_bytes,
                        pikepdf.Dictionary({
                    ...
                    annot['/AP'] = pikepdf.Dictionary({'/N': ap_stream})
                    ap_fields_created += 1

            ap_fields_with_stream = ap_fields_total + ap_fields_created
            try:
                sys.stderr.write(
                    f"[calc/section10] /AP coverage: existing={ap_fields_total} "
                    f"created={ap_fields_created} missing={ap_fields_missing}\n"
                )
            except Exception:
                pass

            if ap_fields_missing == 0 or ap_fields_with_stream > 0 and ap_fields_missing <= ap_fields_with_stream:
                # Most/all fields have /AP — it's safe to turn
                # NeedAppearances off and rely on /CO + calculateNow().
                ...
                # The majority of fields have no /AP (source PDF is a
                # "NeedAppearances=True" AcroForm that relies on viewer-
                # generated appearance streams).  If we turn
                # NeedAppearances off here, every one of those fields
                # renders invisible in Acrobat.  Keep NeedAppearances=True
                # and let Acrobat regenerate appearances itself — the /CO
                # array is still honored for calculation ordering, and
                # Acrobat will run calculations on any value change.
                try:
                    sys.stderr.write(
                        "[calc/section10] Most fields lack /AP — keeping "
                        "NeedAppearances=True so fields stay visible.\n"
                    )
                except Exception:
                    pass
                acroform['/NeedAppearances'] = True
        else:
            # Standard mode (no calculations): keep NeedAppearances True
            # so viewers regenerate appearance streams for any value /
            # style changes we made during this save pass.
            acroform['/NeedAppearances'] = True
        _diag_snapshot("AFTER-SECTION10-CO", pdf)
        _diag_page_fields("AFTER-SECTION10-CO", pdf, 2)

        # 10.5 Visibility Tint for CALCULATED Text/Choice Fields
        # ---------------------------------------------------------------
        # Narrow-scope visibility pass: only touches fields that are part
        # of a calculation (existing /AA/C, newly-added /AA/C, or a cycle-
        # neutralised former calc target).  Non-calc fields keep their
        # original appearance — if the user set the global field fill
        # to "transparent", those fields stay transparent.
        #
        # Why this pass exists: PDFs like DTE's Market-Rate catalog ship
        # calc targets with no /MK/BG and empty/missing /AP, so they
        # render invisible in Acrobat regardless of the user's
        # "Highlight Fields" setting.  Without this pass, calculated
        # totals are a blank rectangle on the page.
        #
        # Previously this pass tinted *every* background-less text field,
        # which broke the user's intentional transparent styling on
        # non-calc fields whenever a single calculation was added.
        try:
            # Darker, unmistakably visible tint (was 0.91/0.94/0.996 — too close to white).
            tint = pikepdf.Array([0.812, 0.890, 0.965])   # light blue, clearly visible on white paper
            tint_rgb = (0.812, 0.890, 0.965)   # matches Acrobat's default Highlight Fields overlay (#CFE3F6)
            border_rgb = (0.20, 0.40, 0.75)            # medium blue outline
            tinted_count = 0
            ap_built_count = 0

            # Build the set of calc-related field names so we only touch
            # calc targets.  Sources:
            #   (a) fields with /AA/C on the widget or its parent
            #   (b) fields in this save's changes.calculations map
            #   (c) fields neutralised in the cycle pass above
            calc_related_names = set()
            try:
                for page in pdf.pages:
                    if '/Annots' not in page:
                        continue
                    for a in page.Annots:
                        nm = str(a.get('/T', ''))
                        if not nm and '/Parent' in a:
                            nm = str(a['/Parent'].get('/T', ''))
                        if not nm:
                            continue
                        aa = a.get('/AA')
                        has_c = aa is not None and '/C' in aa
                        if not has_c and '/Parent' in a:
                            p = a['/Parent']
                            if isinstance(p, pikepdf.Dictionary):
                                ...
                                if p_aa is not None and '/C' in p_aa:
                                    has_c = True
                        if has_c:
                            calc_related_names.add(nm)
            except Exception:
                pass
            # Add names from the save's changes.calculations (name may be
            # pre- or post-rename, so cover both via name_updates).
            try:
                for _cname in (changes.get("calculations", {}) or {}).keys():
                    calc_related_names.add(_cname)
                    mapped = name_updates.get(_cname, _cname)
                    if mapped:
                        calc_related_names.add(mapped)
            except Exception:
                pass
            # Add cycle-neutralised names (they lost /AA/C above but still
            # need visibility treatment so the reader can type into them).
            try:
                for _cyc in cyclic_fields:
                    calc_related_names.add(_cyc)
                    mapped = name_updates.get(_cyc, _cyc)
                    if mapped:
                        calc_related_names.add(mapped)
            except Exception:
                pass
            ...
            for page in pdf.pages:
                ...
                for annot in page.Annots:
                    ...
                    if not f_name and '/Parent' in annot:
                        f_name = str(annot['/Parent'].get('/T', ''))
                    if f_name in transparent_fill_names:
                        continue
                    # NEW: scope to calc-related fields only.  Non-calc
                    # fields retain their original /MK/BG / /AP — this is
                    # what respects the user's transparent styling on the
                    # rest of the form.
                    if f_name not in calc_related_names:
                        continue

                    mk = annot.get('/MK')
                    has_bg = mk is not None and '/BG' in mk
                    existing_ap_n = None
                    if '/AP' in annot:
                        n = annot['/AP'].get('/N')
                        if n is not None and isinstance(n, pikepdf.Stream):
                            existing_ap_n = n

                    # Step 1: add /MK/BG tint if missing
                    if not has_bg:
                        if mk is not None:
                            mk['/BG'] = tint
                        else:
                    ...
                    # Step 1b: if the field has an existing /AP stream,
                    # check whether it actually draws anything visible.
                    # Many source PDFs ship /AP streams that are empty
                    # (no fill, no text), making the field invisible in
                    # Acrobat regardless of /MK/BG.  Heuristic: if the
                    # stream is tiny (< 40 bytes of raw content) or
                    # lacks an 'rg'/'RG' color command, treat it as
                    # empty and rebuild it from the /MK/BG tint.
                    if existing_ap_n is not None:
                        try:
                            raw = bytes(existing_ap_n.read_bytes())
                            is_empty_ap = (
                                len(raw) < 40
                                or (b' rg' not in raw and b' RG' not in raw)
                            )
                            if is_empty_ap:
                                existing_ap_n = None  # trigger rebuild below
                        except Exception:
                            pass

                    # Step 1c: for editable (non-readOnly) fields with no
                    # renderable /AP, build a plain tinted-fill /AP so
                    # the field is visible in Acrobat.  The readOnly path
                    # (Step 2) handles its own stronger appearance.
                    ff_peek = int(annot.get('/Ff', 0)
                                  or (annot.get('/Parent', {}).get('/Ff', 0)
                                      if annot.get('/Parent') else 0))
                    if existing_ap_n is None and not (ff_peek & 1):
                        try:
                            rect = annot.get('/Rect', [0, 0, 10, 10])
                            w = abs(float(rect[2]) - float(rect[0]))
                            h = abs(float(rect[3]) - float(rect[1]))
                            if mk is not None and '/BG' in mk:
                                bg = mk['/BG']
                                try:
                                    rb = float(bg[0]); gb = float(bg[1]); bb = float(bg[2])
                                except Exception:
                                    rb, gb, bb = tint_rgb
                            else:
                                rb, gb, bb = tint_rgb
                            ...
                            ed_stream = pdf.make_indirect(pikepdf.Stream(
                                pdf, ed_bytes,
                                pikepdf.Dictionary({
                                    '/Type':      pikepdf.Name('/XObject'),
                                    '/Subtype':   pikepdf.Name('/Form'),
                            ...
                            annot['/AP'] = pikepdf.Dictionary({'/N': ed_stream})
                            existing_ap_n = ed_stream
                            ap_built_count += 1
                        except Exception as _ed_err:
                            try:
                                sys.stderr.write(f"[visibility] editable AP build failed for {f_name}: {_ed_err}\n")
                            except Exception:
                                pass

                    # Step 2: build a real /AP stream for ReadOnly fields.
                    # Acrobat's NeedAppearances=True regeneration does NOT fire
                    # for ReadOnly empty fields — it just skips drawing them.
                    # The /MK/BG alone is ignored on those fields.  So we must
                    # build an explicit appearance stream containing the tint
                    # fill so readOnly calc fields are visible before their
                    # calculated value gets populated on first open.
                    #
                    # IMPORTANT: we ALWAYS override any existing /AP for a
                    # readOnly field.  Section 10 may have built a nearly-white
                    # /AP from /MK/BG which is indistinguishable from the page
                    # background.  readOnly calc targets need to stand out.
                    ff = int(annot.get('/Ff', 0)
                             or (annot.get('/Parent', {}).get('/Ff', 0)
                                 if annot.get('/Parent') else 0))
                    ...
                    if is_readonly:
                        try:
                            ...
                            w = abs(float(rect[2]) - float(rect[0]))
                            h = abs(float(rect[3]) - float(rect[1]))
                            # For readOnly calc targets use a soft blue tint
                            # that approximates Acrobat's default
                            # "Highlight Existing Fields" overlay colour —
                            # so readOnly calc totals blend in visually
                            # with editable fields when the overlay is on,
                            # and remain at least detectable when it is off.
                            # NO border: the user finds the outline on
                            # calc fields jarring compared to the clean
                            # borderless editable fields around them.
                            rb, gb, bb = tint_rgb
                            ap_bytes = (
                                f"q\n"
                                f"{rb:.3f} {gb:.3f} {bb:.3f} rg\n"
                                f"0 0 {w:.2f} {h:.2f} re f\n"
                                f"Q\n"
                            ).encode()
                            # Build stream with all XObject metadata inline
                            # (matches the working radio-button pattern).
                            # /Resources is REQUIRED by the PDF spec for Form
                            # XObjects — Acrobat silently skips rendering
                            # streams that lack it.
                            ap_stream = pdf.make_indirect(pikepdf.Stream(
                                pdf, ap_bytes,
                                pikepdf.Dictionary({
                                    '/Type':      pikepdf.Name('/XObject'),
                            ...
                            annot['/AP'] = pikepdf.Dictionary({'/N': ap_stream})
                            # Overwrite /MK/BG with the tint so future
                            # appearance regenerations use it.  Do NOT set
                            # /MK/BC or /BS — we want no border.
                            try:
                                if mk is None:
                                    annot['/MK'] = pikepdf.Dictionary({'/BG': tint})
                                    mk = annot['/MK']
                                    ...
                                    mk['/BG'] = tint
                                    # Scrub any pre-existing border entries
                                    # that Section 8.5 or a prior run stamped.
                                    if '/BC' in mk:
                                        del mk['/BC']
                                # Also remove /BS so Acrobat doesn't draw a
                                # fallback border over our /AP.
                                if '/BS' in annot:
                                    del annot['/BS']
                            except Exception:
                                pass
                            ...
                            try:
                                sys.stderr.write(f"[visibility] readOnly AP build failed for {f_name}: {_ap_err}\n")
                            except Exception:
                                pass
            if tinted_count or ap_built_count:
                try:
                    sys.stderr.write(
                        f"[visibility] Tinted {tinted_count} fields, built "
                        f"{ap_built_count} /AP streams for readOnly fields\n"
                    )
                except Exception:
                    pass
        except Exception as _tint_err:
            try:
                sys.stderr.write(f"[visibility] tint pass failed: {_tint_err}\n")
            except Exception:
                pass

        # 10.55 Cleanup of stale calc-tint on non-calc fields
        # ---------------------------------------------------------------
        # Earlier versions (v9 and below) ran Section 10.5 unconditionally on
        # every text/choice field, baking the #CFE3F6 calc tint into /MK/BG
        # *and* /AP on fields that had nothing to do with calculations.
        # PDFs saved by those versions round-trip through the current app
        # with every field still showing that tint — the narrow-scoping we
        # added in v10 only prevents *new* tinting, it doesn't undo the
        # damage baked into existing _Fillable.pdf files.
        #
        # Fix: when the user's global fill is transparent (or unset), strip
        # the specific calc-tint colour from /MK/BG on any field that isn't
        # a calc target.  Also strip the matching fill-rectangle from a
        # baked /AP stream that Section 10 or 10.5 of an older run produced.
        try:
            _global_fill = settings.get('field_background_color', 'transparent')
            _user_transparent_global = (
                not _global_fill or _global_fill == 'transparent'
            )
            if _user_transparent_global:
                # Exact tint we used to spray everywhere (and still emit
                # for calc targets).  Also cover the pale legacy variant
                # 0.910/0.940/0.996 from v1–v8.
                _stale_tints = [
                    (0.812, 0.890, 0.965),
                    (0.910, 0.940, 0.996),
                ]
                ...
                def _build_empty_ap(annot):
                    """Replace or create /AP with an empty, transparent Form XObject."""
                    rect = annot.get('/Rect', [0, 0, 10, 10])
                    w = abs(float(rect[2]) - float(rect[0]))
                    h = abs(float(rect[3]) - float(rect[1]))
                    empty_stream = pdf.make_indirect(pikepdf.Stream(
                        pdf, b"q\nQ\n",
                        pikepdf.Dictionary({
                            '/Type':      pikepdf.Name('/XObject'),
                            '/Subtype':   pikepdf.Name('/Form'),
                    ...
                    annot['/AP'] = pikepdf.Dictionary({'/N': empty_stream})
                ...
                for page in pdf.pages:
                    ...
                    for annot in page.Annots:
                        ...
                        if not f_name and '/Parent' in annot:
                            f_name = str(annot['/Parent'].get('/T', ''))
                        # Never touch calc targets — they have their own
                        # tinted /AP built by Section 10.5.
                        if f_name in calc_related_names:
                            continue

                        mk = annot.get('/MK')
                        # Strip stale calc-tint from /MK/BG if present.
                        stripped_bg = False
                        if mk is not None and '/BG' in mk:
                            try:
                                bg_tup = tuple(round(float(v), 3) for v in mk['/BG'])
                                matched = any(
                                    abs(bg_tup[0] - t[0]) < 0.01
                                    and abs(bg_tup[1] - t[1]) < 0.01
                                    and abs(bg_tup[2] - t[2]) < 0.01
                                    for t in _stale_tints
                                    if len(bg_tup) >= 3
                                )
                            except Exception:
                                matched = False
                        ...
                        # Ensure this field has a Stream /AP so Section 10.6
                        # can flip NeedAppearances=False (required for calc
                        # targets to remain visible — Acrobat's NA-regen
                        # wipes the tinted /AP we built for them).  If the
                        # field has no /AP, or has a non-Stream /AP, build
                        # an empty transparent one.  If we just stripped a
                        # stale-tint /BG, also replace any baked-fill /AP
                        # so the stale tint doesn't render from cache.
                        needs_empty_ap = False
                        if '/AP' not in annot:
                            needs_empty_ap = True
                        else:
                            n = annot['/AP'].get('/N')
                            if not isinstance(n, pikepdf.Stream):
                                ...
                                # Check if the existing AP stream is just
                                # a stale fill rectangle; if so, replace it.
                                try:
                                    raw = bytes(n.read_bytes())
                                    if b' rg' in raw and b' re' in raw and b' f' in raw:
                                        needs_empty_ap = True
                                except Exception:
                                    pass
                        ...
                        if needs_empty_ap:
                            try:
                                ...
                                try:
                                    sys.stderr.write(f"[visibility] empty-AP build failed for {f_name}: {_ap_err}\n")
                                except Exception:
                                    pass

                if _cleared_bg or _cleared_ap or _empty_ap_built:
                    try:
                        sys.stderr.write(
                            f"[visibility] stale-tint cleanup: cleared /MK/BG on "
                            f"{_cleared_bg} non-calc fields, replaced stale /AP on "
                            f"{_cleared_ap}, built empty /AP on {_empty_ap_built} "
                            f"non-calc fields (for full /AP coverage)\n"
                        )
                    except Exception:
                        pass
        except Exception as _cleanup_err:
            try:
                sys.stderr.write(f"[visibility] stale-tint cleanup failed: {_cleanup_err}\n")
            except Exception:
                pass

        # 10.6 Post-tint NeedAppearances / OpenAction re-evaluation
        # ---------------------------------------------------------------
        # Section 10 decided NeedAppearances / OpenAction *before* Section
        # 10.5 ran — at which point most fields still had no /AP (Section
        # 10.5 is the pass that actually builds tint /AP for the majority
        # of text fields).  Consequence: Section 10 always took the "lots
        # of fields lack /AP → keep NeedAppearances=True" branch, which
        # makes Acrobat regenerate appearances on open and wipe the tint
        # we just carefully applied.  The user saw this as the calc-target
        # fields rendering completely blank.
        #
        # Now that Section 10.5 has finished, re-count /AP coverage.  If
        # every (non-transparent) text/choice widget has a Stream /AP,
        # switch NeedAppearances off and install the calculateNow()
        # OpenAction so /CO actually runs on document open.
        if '/CO' in acroform and len(acroform['/CO']) > 0:
            try:
                tot = 0
                with_ap = 0
                for page in pdf.pages:
                    ...
                    for annot in page.Annots:
                        ...
                        if not f_name and '/Parent' in annot:
                            f_name = str(annot['/Parent'].get('/T', ''))
                        if f_name in transparent_fill_names:
                            continue  # transparent fields don't need /AP
                        tot += 1
                        if '/AP' in annot:
                            n = annot['/AP'].get('/N')
                            if n is not None and isinstance(n, pikepdf.Stream):
                                with_ap += 1
                if tot > 0 and with_ap == tot:
                    acroform['/NeedAppearances'] = False
                    # Overwrite any /OpenAction (e.g. original PDF's /GoTo)
                    # with a calculateNow() JavaScript action so Acrobat
                    # runs /CO the moment the document opens.
                    open_js = pikepdf.Dictionary({
                        '/S': pikepdf.Name('/JavaScript'),
                        '/JS': pikepdf.String('this.calculateNow();')
                    })
                    ...
                    try:
                        sys.stderr.write(
                            f"[visibility] post-tint: all {tot} fields have /AP — "
                            f"NeedAppearances=False, OpenAction=calculateNow() installed\n"
                        )
                    except Exception:
                        pass
                else:
                    try:
                        sys.stderr.write(
                            f"[visibility] post-tint coverage: {with_ap}/{tot} — "
                            f"leaving NeedAppearances as-is\n"
                        )
                    except Exception:
                        pass
            except Exception as _na_err:
                try:
                    sys.stderr.write(f"[visibility] post-tint NA re-eval failed: {_na_err}\n")
                except Exception:
                    pass
...
if __name__ == "__main__":
    sys.stderr.write("[MODIFY_FIELDS v1.1.6-CALC-VISIBILITY-PATCH-v10-scoped-to-calc-fields] running from: " + __file__ + "\n")
    sys.stderr.flush()
